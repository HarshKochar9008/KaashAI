...
export default function SignupPage() {
  ...
  const { signup, error, clearError, user, loading } = useAuthStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitting(true);
    const ok = await signup(name, email, password, institution, location);
    setSubmitting(false);
    ...
    <div className="min-h-screen flex items-center justify-center bg-black px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-zinc-500">Get started with KaashAI</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white/[0.04] backdrop-blur-2xl rounded-2xl p-8 border border-white/[0.08] space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-zinc-300 mb-1.5">Institution</label>
              <input
                type="text"
                value={institution}
            ...
            <div>
              <label className="block text-sm font-semibold text-zinc-300 mb-1.5">Location</label>
              <input
                type="text"
                value={location}
